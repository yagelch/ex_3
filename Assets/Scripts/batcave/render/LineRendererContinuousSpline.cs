...
/// <summary>
/// This class is used to test the implementation of ContinuousSpline.
/// It receives all the control points as input at once, but "feeds" them to a
/// ContinuousSpline instance one at a time.
/// </summary>
public class LineRendererContinuousSpline : MonoBehaviour {
    ...
    private void UpdateSpline() {
        ...
        var firstControlPoints = new [] {
            points[0],
            points[1],
            points[2]
        };
        var lastPointToRender = 1;
        var spline = new ContinuousSpline(firstControlPoints);
        // Set starting X.
        var x = points[0].x;
        ...
        if (maintainEvenSegments) {
            ...
            for (int i = 1; i < samplePoints.Length; i++) {
                x += space;
                if (lastPointToRender < points.Length - 2 && x > points[lastPointToRender].x) {
                    // Add the next control point to the spline.
                    ++lastPointToRender;
                    spline.AddControlPoints(points[lastPointToRender + 1]);
                }
            ...
            lastPointToRender = 1;
            spline = new ContinuousSpline(firstControlPoints);
            x = points[0].x;
